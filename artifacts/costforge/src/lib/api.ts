 ...
 *
 * In dev the Vite dev server proxies /backend/* → http://localhost:8000/*
 * (see vite.config.ts), so all requests are made against /backend/api/*.
 *
 * JWT access tokens are stored in localStorage and attached to every
 * request via the Authorization header. Refresh-on-401 is handled in
 * `request()` below.
 */
