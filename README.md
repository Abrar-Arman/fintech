...
CostForge is a FinTech + AI cost prediction platform for freelancers building AI agents. Pick the LLMs / vector DBs / hosts you need, set your usage assumptions, and see what your monthly bill is going to look like — before you ship.
...
# (or, for real production: gunicorn costforge.wsgi --bind 0.0.0.0:8000)
```
...
### Authentication
- The frontend bootstraps a demo user on first load: it tries to `POST /api/auth/register/` with `demo / costforge-demo-2026`, falls back to `POST /api/auth/login/` if the user already exists.
- JWT access + refresh tokens are stored in `localStorage` (`costforge_jwt_access`, `costforge_jwt_refresh`).
